...
import { GET_TIPO_DONACION_BY_ID, UPDATE_TIPO_DONACION } from '@/query/tipodonacionQuery';

export default function UpdateeTipoDonacion({ params }) { // Asegúrate de usar "UpdateEvento" en lugar de "updateEvento"
    const { id } = params;
    const { data } = useSuspenseQuery(GET_TIPO_DONACION_BY_ID, { variables: { id } });
    const tipodonacion = data.tipoDonacionById;
    // console.log(tipodonacion);

    const [isLoading, setIsLoading] = useState(false);

    const [nombre, setNombre] = useState(tipodonacion.nombre);
    const router = useRouter();

    const [UpdateTipoDonacion] = useMutation(UPDATE_TIPO_DONACION);
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true)
        try {
            const { data } = await UpdateTipoDonacion({
                variables: { id, nombre }
            });
            // console.log('Tipo donación actualizado:', data);
            router.push('/tipodonacion');
        } catch (error) {
            console.error('Error al Actualizar el tipo de donación:', error);
        } finally {
        ...
        <DefaultLayout>
            <Breadcrumbs
                breadcrumbs={[
                    ...
                    { label: 'Actualizar tipo donación', href: '/tipodonacion/edit' },
                ]}
            />
