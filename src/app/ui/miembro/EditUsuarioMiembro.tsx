...
import { GET_MIEMBRO_BY_ID, UPDATE_MIEMBRO } from "@/query/miembroQuery";
import { GET_USUARIO_BY_ID, UPDATE_USUARIO } from "@/query/usuarioQuery";


export default function EditUsuarioMiembro({ id }) { // Asegúrate de usar "UpdateEvento" en lugar de "updateEvento"
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      const { data: miembroDataU } = await UpdateMiembro({
        variables: { id, ci, nombre, apellido, foto, fechaNacimiento, celular, genero }
      });

      const miembro = miembroDataU.updateMiembro;

      const { data: usuarioDataU } = await UpdateUsuario({
        variables: { id: usuario.id, rol, estado, correo, password, miembroId: miembro.id }
      });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-4.5 pt-4">
            <button
              ...
              type="submit"
              disabled={isLoading}>{isLoading ? 'Loading...' : 'Actualizar Miembro'}</button>
          </div>
        </form>
      </div>
    </DefaultLayout>
